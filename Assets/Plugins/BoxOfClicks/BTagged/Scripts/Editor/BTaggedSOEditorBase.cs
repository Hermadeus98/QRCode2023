...
namespace BOC.BTagged.EditorTools
{
    ...
    public class BTaggedSOEditorBase : OdinEditor
    {
        ...
        {
            if (lastSearchObject == null || lastSearchObject != target || !BTaggedSORegistry.HasCachedResults(target as BTaggedSO))
            {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (openSceneReferences != null)
            {
                ...
                if (serializedObject.isEditingMultipleObjects)
                {
                    ...
                    {
                        if (!(serializedObject.targetObjects[i] is BTaggedSO)) continue;
                        var tn = serializedObject.targetObjects[i].name;
                        if (hasDir)
                        {
                            while (!tn.StartsWith(nameOfSelected) && nameOfSelected.Contains("/")) nameOfSelected = nameOfSelected.Substring(0, nameOfSelected.LastIndexOf("/"));
                        }
                        else
                        {
                ...
                if (newAssetName != nameOfSelected)
                {
                    ...
                    {
                        if (!(allTargs[i] is BTaggedSO)) continue;
                        var renameGroup = AssetDatabase.LoadMainAssetAtPath(AssetDatabase.GetAssetPath(allTargs[i])) as BTaggedGroupBase;
                        ...
                        BTaggedSOPropertyDrawerBase<BTaggedGroupBase, BTaggedSO>.RenameSOAsset(renameGroup, allTargs[i] as BTaggedSO, newName);
                    }
                }
                if (group != null)
                {
                    string localPath = BTaggedSharedUtils.GetASMDEFDirectory(@"BOC.BTagged.Editor");
                    GUIContent btnContent = new GUIContent(AssetDatabase.LoadAssetAtPath<Texture2D>(Path.Combine(localPath, "BTagged_Delete.png")));
                    GUIStyle deleteBtnStyle = new GUIStyle(GUI.skin.button); deleteBtnStyle.padding = new RectOffset();
                    ...
                    if (GUILayout.Button(btnContent, deleteBtnStyle, GUILayout.Width(20), GUILayout.Height(20)))
                    {
                        if (EditorUtility.DisplayDialog("Deleting " + target, "Are you sure you wish to delete?", "Ok", "Cancel"))
                        {
                            ...
                            BTaggedSOPropertyDrawerBase<BTaggedGroupBase, BTaggedSO>.TryDeleteSO(target as BTaggedSO);
                        }
                        GUIUtility.ExitGUI();
                    }
                ...
                if (BTaggedSharedSettings.ShowHashes)
                {
                    var hashContent = new GUIContent("<color=grey><i>#" + (target as BTaggedSO).Hash.ToString() + "</i></color>",
                        "Click to copy hash.\n\nAs int4:\nx:" + (target as BTaggedSO).Hash.x + "\n"
                        + "y:" + (target as BTaggedSO).Hash.y + "\n"
                        + "z:" + (target as BTaggedSO).Hash.z + "\n"
                        + "w:" + (target as BTaggedSO).Hash.w);
                    //EditorGUILayout.SelectableLabel("<color=grey>" + (target as BTaggedSO).Hash.ToString() + "</color>", hashStyle);
                    bool copyHash = GUILayout.Button(hashContent, hashStyle);
                    if (copyHash)
                    {
                        TextEditor te = new TextEditor();
                        te.text = (target as BTaggedSO).Hash.ToString();
                        te.SelectAll();
                        te.Copy();
                    }
                }
                ...
                var show = EditorGUILayout.BeginFoldoutHeaderGroup(BTaggedSharedSettings.ShowAssetReferences, "Project References");
                if (show != BTaggedSharedSettings.ShowAssetReferences)
                {
                    BTaggedSharedSettings.ShowAssetReferences = show;
                    if (show) RefreshUses(SearchRegistryOption.FullRefresh);
                }

                if (BTaggedSharedSettings.ShowAssetReferences)
                {
                    bool empty = true;
            ...
            {
                BTaggedSharedSettings.ShowAssetReferences = true;
                refresh = true;
            }
        ...
        private void RefreshUses(SearchRegistryOption searchOption)
        {
            ...
            try
            {
                ...
                BTaggedSORegistry.References((target as BTaggedSO), ref allReferences, searchOption);
